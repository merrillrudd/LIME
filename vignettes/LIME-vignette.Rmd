...
```{r}
inits <- create_inputs(lh_list=lh, data_avail_list=input_data, param=FALSE, val=FALSE)
names(inits)
```
...
## Run model
You will use the `runModel` function to run the LIME method. The arguments for applying the LIME method to real data (as opposed to simulation) are:

* `modpath` -- the path to the directory to save results

* `est_sigma` -- which variances to estimate? The analyst can specify a concatenated vector of variance parameter names to estimate. Because this is a mixed effects model, the model should at least estimate the recruitment process error, `log_sigma_R`. Another helpful variance to estimate is the CV for the growth curve, called `log_CV_L`. The analyst could also specify `log_sigma_I`, `log_sigma_C`, and/or `log_F_sd` as the observation errors for the abundance index, catch, or process error on the fishing mortality, respectively. 

* `data_avail` -- this is just a double-check that the model is only fitting to the data types specified by the user. If you don't have catch or an abundance index, they can be left out of the tagged list `input_data`, but a double-check is to tell the model here what data types you have. If a combination of data types are available, these should be separated by an underscore (e.g. "Index_LC"). The options are:
  + Index
  + Catch
  + LC

* `lh_list` -- the list of life history and starting values created above (lh)
...
* `start_f` -- This indicates the first year to estimate fishing mortality. The default is 0 to estimate fishing mortality in all years. If start_f is increased to 10 when the total number of years is 20, this will estimate fishing mortality in years 11-20 and set fishing mortality from years 1-10 as the same value estimated in year 11. 
...
```{r, eval=FALSE}
run <- runModel(modpath=<path>, est_sigma="log_sigma_R", data_avail="LC", lh_list=lh, input_data=input_data, rewrite=TRUE, start_f=0, simulation=FALSE, itervec=NULL)
```
