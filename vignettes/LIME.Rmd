...
output:
  ...
  html_document:
    ...
    toc_float: yes
vignette: |
  %\VignetteIndexEntry{LIME: An R package for simulation andestimation using length data to account for variable fishing mortality and recruitment}  %\VignetteEngine{knitr::rmarkdown}  %\VignetteEncoding{UTF-8}
---
...
There are also several other settings not required but may be useful:
 ...
 - `mgt_type`: relevant if using  the LIME package to project forward a harvest control rule. If `Fdynamics` is input as numeric, `mgt_type` specifies whether `Fdynamics` is the fishing mortality rate to project forward (`mgt_type`='F', default) or catch in weight (`mgt_type`="catch").
...
plot_LCfits(LFlist=list("LF"=true$LF))
```
...
plot_output(Inputs=res_rich$Inputs, Report=res_rich$Report, Sdreport=res_rich$Sdreport, lh=lh, True=true, plot=c("Fish","Rec","SPR","Selex"))
```

```{r, warning=FALSE, message=FALSE, fig.cap="Results from including index and length"}
plot_output(Inputs=res_nocatch$Inputs, Report=res_nocatch$Report, Sdreport=res_nocatch$Sdreport, lh=lh, True=true, plot=c("Fish","Rec","SPR","Selex"))
```

```{r, warning=FALSE, message=FALSE, fig.cap="Results including length only"}
plot_output(Inputs=res_length$Inputs, Report=res_length$Report, Sdreport=res_length$Sdreport, lh=lh, True=true, plot=c("Fish","Rec","SPR","Selex"))
```

```{r, warning=FALSE, message=FALSE, fig.cap="Results including 10 years of length data only"}
plot_output(Inputs=res_length_short$Inputs, Report=res_length_short$Report, Sdreport=res_length_short$Sdreport, lh=lh, True=true, plot=c("Fish","Rec","SPR","Selex"))
```
...
```{r}
plot_output(Inputs=res_length$Inputs, Report=res_length$Report, Sdreport=res_length$Sdreport, lh=lh, True=true, plot=c("Fish","Rec","SPR","Selex"))
plot_output(Inputs=res_alt$Inputs, Report=res_alt$Report, Sdreport=res_alt$Sdreport, lh=lh_alt, True=true, plot=c("Fish","Rec","SPR","Selex"), set_ylim=list("F"=c(0,1), "R"=c(0,5)))
```
...
```{r}
plot_output(Inputs=res_length$Inputs, Report=res_length$Report, Sdreport=res_length$Sdreport, lh=lh, True=true, plot=c("Fish","Rec","SPR","Selex"))
plot_output(Inputs=res_dome$Inputs, Report=res_dome$Report, Sdreport=res_dome$Sdreport, lh=lh_dome, True=true, plot=c("Fish","Rec","SPR","Selex"))
```
