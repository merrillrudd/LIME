...
```
```{r, echo=FALSE}
plot(lh_mf2$S_fl[1,], type="l", lwd=2, xlab="Length", ylab="Selectivity", col=cols[1])
lines(lh_mf2$S_fl[2,], type="l", lwd=2, col=cols[2])
```

Simulate data:
```{r, tidy=TRUE}
true_mf2 <- generate_data(modpath=NULL,
            itervec=1, 
            Fdynamics=c("Constant","Endogenous"),
            Rdynamics="Constant",
            lh=lh_mf2,
...
```{r, fig.width=10, fig.height=6, tidy=TRUE, fig.cap="True population with two fleets, one with constant fishing mortality, and one with open access fishing."}
par(mfrow=c(3,2))
plot(true_mf2$F_y, type="l", lwd=4, xlab="Year", ylab="Fishing mortality", ylim=c(0,max(true_mf2$F_y)*1.1), xaxs="i", yaxs="i", cex.axis=1.5, cex.lab=1.5)
for(i in 1:lh_mf2$nfleets){
  lines(true_mf2$F_ft[i,], col=cols[i])
}
plot(true_mf2$R_t, type="l", lwd=4, xlab="Year", ylab="Recruitment", ylim=c(0,max(true_mf2$R_t)*1.1), xaxs="i", yaxs="i", cex.axis=1.5, cex.lab=1.5)
plot(true_mf2$SPR_t, type="l", lwd=4, xlab="Year", ylab="SPR", ylim=c(0,1), xaxs="i", yaxs="i", cex.axis=1.5, cex.lab=1.5)
plot(true_mf2$D_t, type="l", lwd=4, xlab="Year", ylab="Relative spawning biomass", ylim=c(0,max(c(1,max(true_mf2$D_t)))), xaxs="i", yaxs="i", cex.axis=1.5, cex.lab=1.5)
plot(colSums(true_mf2$Cw_ft), type="l", lwd=4, xlab="Year", ylab="Catch", ylim=c(0,max(colSums(true_mf2$Cw_ft))*1.1), xaxs="i", yaxs="i", cex.axis=1.5, cex.lab=1.5)
for(i in 1:lh_mf1$nfleets){
  lines(true_mf2$Cw_ft[i,], col=cols[i])
}
plot(true_mf2$I_ft[1,], type="l", xlab="Year", ylab="Abundance index", ylim=c(0,max(true_mf2$I_ft)*1.1), xaxs="i", yaxs="i", cex.axis=1.5, cex.lab=1.5, col=cols[1])
lines(true_mf2$I_ft[2,], col=cols[2])
```


Move the generated data from an array to a list to plot the data.
```{r, echo=FALSE}
LF_list <- lapply(1:true_mf2$nfleets, function(x){
  return(true_mf2$LF[,,x])
})
```
...
```{r, tidy=TRUE}
data_mf2 <- list("years"=1:true_mf2$Nyears, "LF"=true_mf2$LF, "I_ft"=true_mf2$I_ft, "C_ft"=true_mf2$Cw_ft, "neff_ft"=true_mf2$obs_per_year)
inputs_mf2 <- create_inputs(lh=lh_mf2, input_data=data_mf2)
```


For now, use the multinomial distribution for multiple fleets, specifying `LFdist=0`.
```{r, tidy=TRUE, results="hide", echo=TRUE}
rich_mf2 <- run_LIME(modpath=NULL, 
                  input=inputs_mf2,
                  data_avail="Index_Catch_LC",
                  C_type=2,
                  vals_selex_ft = lh_mf2$S_fl,
                  LFdist=0)

gradient <- rich_mf2$opt$max_gradient <= 0.001
hessian <- rich_mf2$Sdreport$pdHess
hessian==TRUE & gradient == TRUE
```


```{r, tidy=TRUE, fig.height=6, fig.width=8, fig.cap="Fits to length composition data for the data-rich LIME model run with two fleets, one with logistic and one dome-shaped selectivity."}
plot_LCfits(Inputs=rich_mf2$Inputs,
            Report=rich_mf2$Report)
```

```{r, tidy=TRUE, fig.height=6, fig.width=8, fig.cap="LIME estimates of key population parameters (green) compared to the truth (black) for the data-rich case with two fleets each with logistic selectivity."}
plot_output(Inputs=rich_mf2$Inputs,
            Report=rich_mf2$Report,
            Sdreport=rich_mf2$Sdreport,
            lh=lh_mf2,
            True=true_mf2,
            plot=c("Fish","Rec","SPR","ML","SB","Selex"),
            set_ylim=list("SPR"=c(0,1)))
