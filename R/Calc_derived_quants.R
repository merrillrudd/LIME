...
calc_derived_quants = function( Obj, lh ){
  ...
  SPR <- with(lh, calc_ref(ages=ages, Mat_a=Mat_a, W_a=W_a, M=M, S_fa=Report$S_fa, F=Report$F_t[length(Report$F_t)], ref=FALSE, type="SPR"))
  F30 <- tryCatch(with(lh_new, uniroot(calc_ref, lower=0, upper=50, ages=ages, Mat_a=Mat_a, W_a=W_a, M=M, S_fa=Report$S_fa, ref=0.3, type="SPR")$root), error=function(e) NA) * lh_new$nseasons
  F40 <- tryCatch(with(lh_new, uniroot(calc_ref, lower=0, upper=50, ages=ages, Mat_a=Mat_a, W_a=W_a, M=M, S_fa=Report$S_fa, ref=0.4, type="SPR")$root), error=function(e) NA) * lh_new$nseasons
  FF30 <- FF40 <- NULL
